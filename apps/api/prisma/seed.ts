...
async function main() {
  ...
  // 4. Create 20 Players (assigned to agents)
  const players = [];
  for (let i = 1; i <= 20; i++) {
  ...
  // 5. Create 10 Campaigns (mix of agent/affiliate)
  const campaigns = [];
  for (let i = 1; i <= 10; i++) {
    const isAgent = i <= 5;
  ...
  // 6. Generate 30 days of synthetic data
  const now = new Date();
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);

  // Track events: clicks → registrations → deposits
  ...
  for (let day = 0; day < 30; day++) {
    ...
    // Generate clicks (more for affiliate campaigns)
    ...
    // Generate registrations (10-20% of clicks)
    ...
    // Generate deposits (30-50% of registrations)
    ...
    // Generate losses for agent commissions
    for (const player of players.slice(0, 10)) {
      if (player.agentId) {
        ...
        await commissionService.processAgentCommissionFromLosses(
          player.agentId,
          player.id,
          lossAmount,
          date
  ...
  // 7. Process affiliate commissions for FTDs and deposits
